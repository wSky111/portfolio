...
<body>
    ...
    if($noheb == null){
        $sql="SELECT * FROM `resa` WHERE DATEDEBSEM = :DATEDEBSEM ";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':DATEDEBSEM', $datedebsem);
        $stmt->execute();
    }
    else if($datedebsem == null){
        $sql="SELECT * FROM `resa` WHERE NOHEB = :NOHEB ";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':NOHEB', $NOHEB);
        $stmt->execute();
    }
    else {
    $sql="SELECT * FROM `resa` WHERE DATEDEBSEM = :DATEDEBSEM AND NOHEB = :NOHEB ";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':DATEDEBSEM', $datedebsem);
    $stmt->bindParam(':NOHEB', $NOHEB);
    $stmt->execute();
    ...
    <td><?php echo"<a href='../infoshebergement.php?num=",$enreg['NORESA'],"''>Modifier</a>";?></td>        
